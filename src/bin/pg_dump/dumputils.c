				 ...
				 PQExpBuffer sql)
{
	...
	if (strcmp(type, "DIRECTORY") == 0)
		appendPQExpBuffer(firstsql, "REVOKE ON DIRECTORY %s ALL FROM PUBLIC;\n",
						  name);
	else
	{
	...
	for (i = 0; i < naclitems; i++)
	{
		...
		if (privs->len > 0 || privswgo->len > 0)
		{
				...
				&& strcmp(grantor->data, owner) == 0)
			{
				...
				{
					/* Handle special GRANT syntax for DIRECTORY */
					if (strcmp(type, "DIRECTORY") == 0)
						appendPQExpBuffer(firstsql, "REVOKE ON DIRECTORY %s ALL FROM %s;\n",
			...
			else
			{
				...
				if (privs->len > 0)
				{
					/* Handle special GRANT syntax for DIRECTORY */
					if (strcmp(type, "DIRECTORY") == 0)
						appendPQExpBuffer(secondsql, "%sGRANT ON DIRECTORY %s %s TO ",
										  prefix, name, privs->data);
					else
