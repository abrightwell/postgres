...
acldefault(GrantObjectType objtype, Oid ownerId)
{
	...
	switch (objtype)
	{
		...
		case ACL_OBJECT_DIRECTORY:
			world_default = ACL_NO_RIGHTS;
			owner_default = ACL_ALL_RIGHTS_DIRECTORY;
			break;
		default:
...
/*
 * Is member a member of role, not considering superuserness or ADMIN?
 *
 * This is identical to is_member_of_role except we ignore superuser and ADMIN
 * status.
 */
bool
